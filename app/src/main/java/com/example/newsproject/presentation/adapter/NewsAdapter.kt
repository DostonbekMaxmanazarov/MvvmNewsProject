...
const val BREAKING_NEWS = R.layout.item_child_news
const val BREAKING_NEWS_TITLE = R.layout.item_breaking_news_title
const val NEWS_TOP_STORIES = R.layout.item_news_top_stories
...
class NewsAdapter : RecyclerView.Adapter<BaseVH>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseVH {
        ...
        return when (viewType) {
            BREAKING_NEWS -> BaseVH.BreakingNewsVH(ItemChildNewsBinding.bind(view), parent.context)
            BREAKING_NEWS_TITLE -> BaseVH.BreakingNewsTitleVH(ItemBreakingNewsTitleBinding.bind(view))
            NEWS_TOP_STORIES -> BaseVH.NewsTopStoriesVH(ItemNewsTopStoriesBinding.bind(view))
    ...
    override fun onBindViewHolder(holder: BaseVH, position: Int) {
        when (holder) {
            ...
            is BaseVH.NewsTopStoriesVH -> holder.bind(mList[position] as TopStoriesNewsModel)
    ...
    override fun getItemViewType(position: Int) = when (mList[position]) {
        ...
        is TopStoriesNewsModel -> NEWS_TOP_STORIES_TITLE
        else -> throw IllegalArgumentException("wrong item type")
    }

    override fun getItemCount() = mList.size

    fun submitList(data: List<BaseNewsModel>) {
        mList.clear()
        mList.addAll(data)
